...
from multilayer_perceptron.model import MultilayerPerceptron
...
def show_predictions_grid(path_save, model, test_loader, class_names):
    ...
    with torch.no_grad():
        images_flatten = torch.flatten(images, 1).to(device)
...
def train(name: str, activation_default_function, activation_output_function, n1, n2):

    print(name, "Training...")
    model = MultilayerPerceptron(784, [n2 for i in range(n1)], 10,
                                 activation_default_function,
    ...
    for epoch in range(n_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            model.train()
            inputs, labels = data
            inputs = torch.flatten(inputs, 1).to(device)
...
if __name__ == '__main__':
    ...
    tests = [
        ("ReLu_ReLu_3_16", torch.nn.ReLU(), torch.nn.ReLU(), 3, 16),
        ("ReLu_ReLu_3_32", torch.nn.ReLU(), torch.nn.ReLU(), 3, 32),
        ("ReLu_ReLu_3_64", torch.nn.ReLU(), torch.nn.ReLU(), 3, 64),
        ("ReLu_ReLu_6_16", torch.nn.ReLU(), torch.nn.ReLU(), 6, 16),
        ("ReLu_ReLu_6_32", torch.nn.ReLU(), torch.nn.ReLU(), 6, 32),
        ("ReLu_ReLu_6_64", torch.nn.ReLU(), torch.nn.ReLU(), 6, 64),
    ]
    with Pool(9) as p:
        p.map(train_runner, tests)
